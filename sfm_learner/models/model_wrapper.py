...
from sfm_learner.datasets.transforms import get_transforms
from sfm_learner.utils.depth import inv2depth, post_process_inv_depth, compute_depth_metrics
from sfm_learner.utils.horovod import print0, world_size, rank, on_rank_0
from sfm_learner.utils.image import flip_lr
from sfm_learner.utils.load import load_class, load_class_args_create, \
    load_network, filter_args
from sfm_learner.utils.logging import pcolor
from sfm_learner.utils.reduce import all_reduce_metrics, reduce_dict, \
    create_dict, average_loss_and_metrics
from sfm_learner.utils.save import save_depth
from sfm_learner.models.model_utils import stack_batch
...
class ModelWrapper(torch.nn.Module):
    ...
    def __init__(self, config, resume=None, logger=None, load_datasets=True):
        ...
        # Prepare model
        self.prepare_model(resume, config.arch.continue_ckpt_epochs)
    ...
    def prepare_model(self, resume=None, continue_ckpt_pochs = False):
        ...
        if resume:
            ...
            if continue_ckpt_pochs and 'epoch' in resume:
    ...
    def configure_optimizers(self):
        ...
        if self.resume and self.config.arch.continue_ckpt_epochs:
...
def setup_dataset(config, mode, requirements, **kwargs):
    ...
    for i in range(len(config.split)):
        ...
        if config.dataset[i] == 'KITTI':
            from sfm_learner.datasets.kitti_dataset import KITTIDataset
        ...
        # DGP dataset
            ...
            from sfm_learner.datasets.dgp_dataset import DGPDataset
            dataset = DGPDataset(
        ...
        # Image dataset
            ...
            from sfm_learner.datasets.image_dataset import ImageDataset
            dataset = ImageDataset(
                config.path[i], config.split[i],
                **dataset_args, **dataset_args_i,
            )
        elif config.dataset[i] == 'CityScapes':
            from sfm_learner.datasets.cityscapes_dataset import CityScapesDataset
